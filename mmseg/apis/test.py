...
def single_gpu_test(model,
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        with torch.no_grad():
            # segmap = dataset.get_gt_seg_map_by_idx(batch_indices[0])
...
def vis_output(model, data_loader, config_name, num_vis, 
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        ...
        with torch.no_grad():
            seg_logit = model(return_loss=False, return_logit=True, **data)
...
def ohem_test(model,
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        with torch.no_grad():
            seg_map = dataset.get_gt_seg_map_by_idx(batch_indices[0])
        ...
        if out_dir:
            ...
            for img, img_meta in zip(imgs, img_metas):
                ...
                model.module.show_ohem(
                    img_show,
                    seg_logit,
                    seg_map,
                    out_file=out_file)

        batch_size = len(seg_logit)
        for _ in range(batch_size):
            prog_bar.update()
