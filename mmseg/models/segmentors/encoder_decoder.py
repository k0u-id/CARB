...
class EncoderDecoder(BaseSegmentor):
    ...
    def __init__(self,
        ...
        self.feed_img_to_decode_head = feed_img_to_decode_head
        self.mix_labels = torch.tensor(mix_labels, device='cuda')
    ...
    def _decode_head_forward_train(self, x, img_metas, gt_semantic_seg, img=None, img_label=None):
        """Run forward function and calculate loss for decode head in
        training."""
        losses = dict()
        if self.feed_img_to_decode_head:
            loss_decode = self.decode_head.forward_train(x, img_metas, gt_semantic_seg,
                                                            self.train_cfg, img, img_label)
        else:
            loss_decode = self.decode_head.forward_train(x, img_metas, gt_semantic_seg,
                                                            self.train_cfg)
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        img_label = torch.tensor([img_metas[j]['img_label'] for j in range(len(img_metas))], device=img.device)
        x = self.extract_feat(img)

        losses = dict()

        if self.feed_img_to_decode_head:
            loss_decode = self._decode_head_forward_train(x, img_metas, gt_semantic_seg, img, img_label)
        else:
    ...
    def simple_test(self, img, img_meta, rescale=True, return_logit=False):
        ...
        # values, indexs = torch.topk(seg_logit, k=2, dim=1)
        # diff = values[:,0,:,:] - values[:,1,:,:]
        # seg_pred[diff<values[:,0,:,:]*0.1] = 255
        ###############################
        # seg_max, _ = torch.max(seg_logit, dim=1)
        # seg_pred[seg_max<0.06] = 255
        # print(seg_max.shape) #[1, 1, 1024, 2048]
        ###############################
        # seg_max = F.adaptive_max_pool2d(seg_logit, (1, 1)) #[1, 19, 1, 1]
        # mask = torch.zeros_like(seg_pred)
        # # seg_logit = torch.max(seg_logit, dim=1)
        # for i in range(0, 19):
        #     mask[seg_logit[:,i,:,:] > seg_max[:,i,:,:]*0.95] = 255
        # seg_pred[mask != 255] = 255      
        ################################
    ...
    def aug_test(self, imgs, img_metas, rescale=True):
        ...
        ###############################
        # this part is edited part
        # seg_logit /= F.adaptive_max_pool2d(seg_logit, (1, 1)) + 1e-5
        ###############################
        ###############################
        seg_pred = seg_logit.argmax(dim=1)
        # seg_max = F.adaptive_max_pool2d(seg_logit, (1, 1)) #[1, 19, 1, 1]
