...
class BaseSegmentor(BaseModule, metaclass=ABCMeta):
    ...
    def show_ohem(self,
        ...
        logit = torch.tensor(logit, device='cuda')
        segmap = torch.tensor(segmap, device='cuda')

        # print(logit.shape)
        # print(segmap.shape)

        loss = F.cross_entropy(logit, segmap.unsqueeze(0), weight=None, reduction='none', ignore_index=255)
        colormap = cv2.COLORMAP_JET

        # print(loss.shape)

        heatmap = cv2.applyColorMap(np.uint8(255 * loss.squeeze(0).cpu().numpy()), colormap)
        ...
        if out_file is None:
            raise Exception("always need to input out_file")
