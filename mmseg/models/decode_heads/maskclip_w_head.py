...
class WeakMaskClipHead(BaseDecodeHead):
    def __init__(self, text_categories, text_channels, text_embeddings_path,
        ...
        super(WeakMaskClipHead, self).__init__(**kwargs)
    ...
    def init_weights(self):
        super(WeakMaskClipHead, self).init_weights()
        if self.text_embeddings_path is None:
            nn.init.normal_(self.text_embeddings, mean=0.0, std=0.01)
        else:
            self.load_text_embeddings()
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        raise RuntimeError('MaskClip is not trainable. Try MaskClip+ instea.')
    def forward_test(self, inputs, img_metas, test_cfg):
        ...
        ###########################
        # prob = torch.softmax(output * self.logit_scale, dim=1)
        # entropy = -torch.sum(prob * torch.log(prob + 1e-10), dim=1)
        # ###########################
        ###############################
        # stuff_label = torch.ones((1, 11), device=output.device)
        # thing_label = torch.zeros((1, 8), device=output.device)
        # thing_stuff_filter = torch.cat((stuff_label, thing_label), dim=1)
        # output = output * thing_stuff_filter.reshape(1, -1, 1, 1)
        ###############################
